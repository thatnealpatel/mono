...
func (p *precomputedAutP3) splitIntoGoodOrders(h *AutP3) (*AutP3, *AutP3) {
	for {
		h1 := NewAutP3Rand()
		h2 := h1.Inv().Mul(h)
		if _, ok1 := p.goodOrders[h1.mustOrder()]; ok1 {
			if _, ok2 := p.goodOrders[h2.mustOrder()]; ok2 {
				return h1, h2
			}
		}
	}
...
// Order returns the order of b in the Bimonster. It
// returns an error if a Monster component order could
// not be determined within the search bound of
// orderElementGx0.
func (b *BiMM) Order() (int, error) {
	o1, o2, s := b.Orders()
	if o1 == 0 || o2 == 0 {
		return 0, errors.New("cgt: BiMM.Order: a Monster component order is 0 " +
			"(order exceeded the search bound in orderElementGx0)")
	}
	return s * o1 * o2 / gcdInt(o1, o2), nil
}
