...
// representation. It returns an error if the sparse
// conversion engine reports failure, which signals a
// corrupted vector buffer.
func (v *MMVector) AsSparse() ([]uint32, error) {
	sp := make([]uint32, mmAuxXlenV)
	length := mmvToSparse(v.p, v.data, sp)
	if length < 0 {
		return nil, fmt.Errorf("cgt: AsSparse failed with code %d", length)
	}
	return sp[:length], nil
}

// AsTuples returns the nonzero entries as tuples
// (factor, tag, i0, i1). C/Python sparse_to_tuples.
// AsTuples panics if the sparse conversion engine
// fails (see AsSparse), which signals a corrupted
// vector buffer.
...
// vector_mul_exp via mm_op_word. It panics if the
// underlying word-application engine reports a
// failure, which signals a malformed word g.
