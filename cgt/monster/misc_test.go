...
func TestBiMMOrder(t *testing.T) {
	...
	words := [][]int{{}, {0}, {0, 13}}
	for _, w := range words {
		got, err := P3BiMM(w).Order()
		if err != nil {
			t.Fatalf("P3BiMM(%v).Order(): %v", w, err)
		}
...
func TestBiMMSpiderOrder(t *testing.T) {
	t.Parallel()
	got, err := P3BiMM([]int{0, 14, 15, 0, 16, 17, 0, 18, 19}).Order()
	if err != nil {
		t.Fatalf("spider order: %v", err)
	}
...
func TestAutP3Type(t *testing.T) {
	...
	go1, err := g.Order()
	if err != nil {
		t.Fatalf("AutP3Rand.Order(): %v", err)
	}
	if go1 <= 0 {
		t.Fatalf("AutP3Rand.Order() = %d, want > 0", go1)
	}
	h := NewAutP3Rand()
	gh := g.Mul(h)
	gho, err := gh.Order()
	if err != nil {
		t.Fatalf("AutP3.Mul.Order(): %v", err)
	}
	if gho <= 0 {
		t.Fatalf("AutP3.Mul.Order() = %d, want > 0", gho)
	}
	p := gh.Perm()
	if len(p) == 0 {
...
func TestBiMMGroupOps(t *testing.T) {
	t.Parallel()
	id := BiMMIdentity()
	idOrder, err := id.Order()
	if err != nil {
		t.Fatalf("BiMMIdentity.Order(): %v", err)
	}
	if idOrder != 1 {
		t.Fatalf("BiMMIdentity.Order() = %d, want 1", idOrder)
	}
	b := P3BiMM([]int{0})
