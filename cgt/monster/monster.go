...
// 255-bit integer produced by AsInt. It returns an
// error if n does not encode a monster element,
// mirroring the ValueError raised by the Python
// MM_from_int.
func MMFromInt(n uint64) (*MM, error) {
	g, err := reduce.ExpandInt(n)
	if err != nil {
		return nil, fmt.Errorf("MMFromInt: %w", err)
	}
	return &MM{data: g}, nil
}
...
// (an expected per-trial miss), conjugateInvolution's
// G_x0 fast path now reports ok=false directly (via
// ConjugateInvolutionErr), so no recover is needed.
...
// AxisFor returns the 2A axis v^+ . g. It returns an
// error if the standard axis vector v^+ fails to
// parse, which can only happen on a corrupted build.
func AxisFor(g *MM) (*Axis, error) {
	base, err := ParseVector(15, axisV15)
	if err != nil {
		return nil, fmt.Errorf("AxisFor: %w", err)
	}
	return &Axis{
		g: g,
		v: base.Mul(g.Mmdata()),
...
// returns a G_x0 word. It returns an error if the
// axis cannot be rebased onto the standard axis,
// e.g. when a is not a 2A axis.
func (a *Axis) ReduceGx0() (*MM, error) {
	g := rebaseAxis(a.v)
	if g == nil {
		return nil, errors.New("ReduceGx0: rebasing of axis failed")
	}
	return g, nil
}
