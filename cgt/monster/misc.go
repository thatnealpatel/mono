...
// Order returns the order of g in AutP3. It returns
// an error if no order in 1..13 is found, which
// cannot happen for a well-formed element of
// Aut(PG(2,3)) (whose element orders divide its
// exponent and are at most 13).
func (g *AutP3) Order() (int, error) {
	...
	return 0, errors.New("cgt: cannot compute order in AutP3")
}

// mustOrder returns the order of g in AutP3 and
// panics if it cannot be determined. It is the
// internal helper for the bimm machinery, where g is
// always a well-formed element of Aut(PG(2,3)) and a
// failure signals a genuine bug.
func (g *AutP3) mustOrder() int {
	o, err := g.Order()
	if err != nil {
		panic(err.Error())
	}
...
func conjugateInvolution(g *MM, check bool, ntrials int) (int, *MM, bool) {
	...
	if h := g.checkInGx0(); h != nil {
		...
		it, hw, err := elem.ConjugateInvolutionErr()
		if err != nil {
			// g is in G_x0 but not an involution; report
			// failure via ok=false instead of panicking, so
			// per-trial callers (tryConjugateInvolution)
			// need no recover.
			return 0, nil, false
		}
...
// always a word in G_x0 generators; a non-involution
// is the only expected failure and the trial loop
// treats it as a skip via ok=false. It uses the
// error-returning ConjugateInvolutionErr so the
// expected miss is reported as a value rather than a
// recovered panic.
