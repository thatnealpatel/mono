// Package mmindex ports mm_index.c from the C
// mmgroup mm_basics layer: the index-conversion
// family that maps between the external, internal,
// sparse, and Leech-lattice-mod-2 labelings of the
// 196884-dimensional Monster representation. It sits
// below both the mm (MMVector) and reduce layers and
// depends only on mat24.
package mmindex
...
func IndexSparseToExtern(sp uint32) int {
	...
	case 1: // A
		...
		return int((tag-1)*276 + XofsA + ((i*i - i) >> 1) + j)
	case 4: // T
		if i >= 759 {
			return -1
		}
		return int(uint32(XofsT) + (i << 6) + j)
	case 5, 6, 7: // X, Z, Y
		if j >= 24 {
			return -1
		}
		return int((24*((tag<<11)+i) - 0x3c000) + XofsX + j)
	default:
		return -1
	}
}
...
func IndexSparseToIntern(sp uint32) int {
	...
	case 4: // T
		...
		return int(OfsT + (i << 6) + j)
	case 5, 6, 7: // X, Z, Y
		if j >= 24 {
			return -1
		}
		return int(OfsX + 32*(((tag-5)<<11)+i) + j)
	default:
		return -1
	}
}
