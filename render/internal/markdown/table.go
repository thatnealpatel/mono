...
func isTableStart(hdr1, delim1 string, math bool) bool {
	...
	return col == tableCountMath(tableTrimOuter(hdr1), math)
}

// tableCount returns the number of columns in the row.
func tableCount(row tableTrimmed) int {
	return tableCountMath(row, false)
}

func tableCountMath(row tableTrimmed, math bool) int {
	col := 1
	prev := byte(0)
	source := string(row)
	code := newTableCodeTracker(source)
	for i := 0; i < len(row); i++ {
		if math && !code.contains(i) && (source[i] == '$' || source[i] == '\\') {
			if _, end, ok := parseInlineMath(nil, source, i); ok {
				i = end - 1
				prev = 0
				continue
			}
...
type tableBacktickRun struct {
	start int
	end   int
}

type tableCodeCandidate struct {
	closeEnd int
}

type tableCodeTracker struct {
	candidates []tableCodeCandidate
	end        int
}

func newTableCodeTracker(source string) *tableCodeTracker {
	var runs []tableBacktickRun
	for start := 0; start < len(source); {
		...
		runs = append(runs, tableBacktickRun{start: start, end: end})
		start = end
	}

	candidates := make([]tableCodeCandidate, len(source))
	var last [maxBackticks]int
	for i := range last {
		last[i] = -1
	}
	for i := len(runs) - 1; i >= 0; i-- {
		...
		if 0 < openN && openN <= maxBackticks && last[openN-1] >= 0 {
			candidates[openStart].closeEnd = runs[last[openN-1]].end
		}
		if n <= maxBackticks {
			last[n-1] = i
		}
	}
	return &tableCodeTracker{candidates: candidates}
}

func (t *tableCodeTracker) contains(offset int) bool {
	if offset < t.end {
		return true
	}
	if offset < len(t.candidates) && t.candidates[offset].closeEnd > 0 {
		t.end = t.candidates[offset].closeEnd
		return true
	}
	return false
}

func (t *tableCodeTracker) reset() {
	t.end = 0
}
...
func (b *tableBuilder) build(p *parser) Block {
	...
	width := tableCountMath(b.hdr, p.Math)
...
func (b *tableBuilder) parseRow(p *parser, row tableTrimmed, line int, width int) []*Text {
	...
	code := newTableCodeTracker(source)
	for i := 0; i < len(row); i++ {
		if p.Math && !code.contains(i) && (source[i] == '$' || source[i] == '\\') {
			if _, end, ok := parseInlineMath(nil, source, i); ok {
				i = end - 1
				continue
			}
		...
		if c == '|' {
			text := strings.Trim(string(row[start:i]), " \t\v\f")
			out = append(out, p.newText(pos, tableUnescapeMath(text, p.Math)))
			if len(out) == width {
	...
	text := strings.Trim(string(row[start:]), " \t\v\f")
	out = append(out, p.newText(pos, tableUnescapeMath(text, p.Math)))
	for len(out) < width {
...
// tableUnescapeMath unescapes table separators outside math spans.
func tableUnescapeMath(text string, math bool) string {
	var out strings.Builder
	out.Grow(len(text))
	code := newTableCodeTracker(text)
	for i := 0; i < len(text); i++ {
		if math && !code.contains(i) && (text[i] == '$' || text[i] == '\\') {
			if _, end, ok := parseInlineMath(nil, text, i); ok {
