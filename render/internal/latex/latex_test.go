...
func TestParseRejectsMalformedInput(t *testing.T) {
	for _, expr := range []string{`\frac{x}`, `{x`, `x^`, `\left(x`, `\middle|`, `\begin{cases}x`, `\Big x`, `\Big\alpha`, `\left\alpha x\right)`, `\left(x\right\beta`, `\mathrel`, `\mathbin`, `\mathscr`, `\widetilde`, `\phantom`} {
		_, err := Parse(expr)
