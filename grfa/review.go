...
// patchSetLevel is Gerrit's pseudo-path for patch-set-level
// comment threads: a real thread with no file location, which is
// what gives -resolved meaning.
...
// cmdComment posts one patch-set-level comment thread, or a reply
// into an existing published thread, and then displays the
// refreshed change. Approvers are rechecked by the refetch; an
// old approval is never inferred.
func (c *cli) cmdComment(ctx context.Context, change, message string, args []string) error {
	...
	if opts.reply == "" {
		...
		if !ok {
			// Fail without posting rather than searching
			// elsewhere; an id missing from the change-level
			// list is not a target at all.
			return fmt.Errorf("reply: no published comment with id %q (change messages are not reply targets)", opts.reply)
		}
		// Reply against the target comment's own patch set: a
		// reply through revisions/current would carry the
		// original location against a revision where the
		// commented region may no longer exist.
		revision = target.Info.CommitID
		path = target.Path
		in.InReplyTo = target.Info.ID
		// Copy the target's positional metadata; absent fields
		// stay absent instead of being invented. side defaults
		// to REVISION server-side, and line is ignored when
		// range is present, so line and range travel together
		// with the target's own values.
...
// pathComment is a published comment paired with the map key it
// appeared under in the change-level comment response.
type pathComment struct {
	Path string
	Info commentInfo
}

// flattenComments returns every published comment, paths sorted
// for deterministic output, comments in server order within a
// path.
...
// findComment matches a comment id verbatim, exactly as view
// printed it, against the decoded change-level comment list.
// IDs are opaque strings: never normalized, re-cased, or
// re-encoded.
...
// unresolvedThreads reports the unresolved-thread count. The
// server's count is authoritative when present; otherwise it is
// derived from the published threads, whose resolution state
// lives in their chronologically last comment.
...
// accountName renders an account for display, preferring the
// most human-readable field the server provided.
...
// renderChange prints the change in a compact, deterministic
// layout: comment ids are printed exactly as received, since they
// are the only handle -reply accepts.
func renderChange(w io.Writer, d *changeDetail, comments map[string][]commentInfo) error {
	...
	for _, pc := range flattenComments(comments) {
		...
		// side=PARENT alone does not distinguish parent 1 from
		// parent 2, so the merge-parent number is printed too.
		if c.Side != "" {
